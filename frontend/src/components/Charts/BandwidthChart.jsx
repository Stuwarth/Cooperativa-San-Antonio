...
export default function BandwidthChart({ bandwidthHistory = [] }) {
  const data = {
    labels: bandwidthHistory.map((_, i) => `${i}s`),
    datasets: [{
  ...
  return (
    <div className="panel" style={{ padding: 'var(--smat-space-4)', height: '160px', display: 'flex', flexDirection: 'column' }}>
      <span className="text-mono text-muted" style={{ fontSize: 'var(--smat-text-xs)', marginBottom: 'var(--smat-space-3)' }}>ANCHO DE BANDA EN VIVO (Mbps)</span>
